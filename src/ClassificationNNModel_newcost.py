...
class ClassificationNNModel(LaIRNNModel):
    ...
    def add_loss_op(self, logits):
        ...
        buckets = tf.argmax(pred, axis=1) 
        buckets = tf.cast(buckets, tf.float32)
        labels = tf.cast(self.labels_placeholder, tf.float32)
        diffs = tf.exp(tf.subtract(buckets, labels))
        weighted_diffs = tf.multiply(tf.reduce_max(pred, axis=1), diffs)
        return tf.reduce_sum(weighted_diffs) + tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=self.labels_placeholder, name='softmax')
